...
describe("BondFixedExpiryOSDA Tests", () => {
    ...
    describe("Fixed-expiry market", () => {
        beforeEach(async () => {
            ...
            // Deploy Auctioner
            const Auctioner = await ethers.getContractFactory("BondFixedExpiryOSDA");
            auctioner = await Auctioner.deploy(teller.address, aggregator.address, ownerSigner.address, authority.address); // teller, aggregator, owner, authority
        ...
        it("should create a fixed-expiry market, transfering the payout tokens ", async () => {
            ...
            // Call create market
            const tx = await auctioner.createMarket(encodedParams);

            // Get bond token from events, it's needed later to claim
            let receipt = await tx.wait();
            // Remove leading zeros from event - This is only needed during the test
            const withoutPrefix = receipt.events[2].data.slice(2);
            const cleanedData = withoutPrefix.replace(/^0+/, "");
            // Convert to address format
            const bondToken = `0x${cleanedData}`;
