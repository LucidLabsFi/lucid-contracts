...
describe("BondFixedTermSDA Tests", () => {
    let vestingLength = 86400; // 1 day. Since market is fixed term, vesting is vesting length in seconds
    ...
    describe("Create fixed-term dutch auction market", () => {
        beforeEach(async () => {
            // Deploy Teller
            const Teller = await ethers.getContractFactory("BondFixedTermTeller");
            teller = await Teller.deploy(user1Signer.address, aggregator.address, ownerSigner.address, authority.address, vesting.address); // fees collector, aggregator, owner, authority, vesting address

            // Deploy Auctioner
            const Auctioner = await ethers.getContractFactory("BondFixedTermSDA");
            auctioner = await Auctioner.deploy(teller.address, aggregator.address, ownerSigner.address, authority.address); // teller, aggregator, owner, authority
    ...
    describe("Fixed-term linear vesting bonds", () => {
        ...
        it("should revert if the linear duration is less than the MIN_VESTING_DURATION", async () => {
            const invalidLinearDuration = 300; // 5 mins in seconds
            //
            // Create market on Auctioner
            //
            // Encode function params
