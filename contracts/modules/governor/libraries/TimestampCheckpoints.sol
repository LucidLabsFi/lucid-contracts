 ...
 *
 * @notice Modifier from OpenZeppelin Contracts (last updated v4.9.0) (utils/Checkpoints.sol)
 * Modified to use timestamps instead of block numbers
 *
...
library TimestampCheckpoints {
     ...
     * timestamp, the requested timestamp must be in the past, excluding the current timestamp.
     */
    function getAtTimestamp(History storage self, uint256 timestamp) internal view returns (uint256) {
        ...
        uint32 key = SafeCast.toUint32(timestamp);
    ...
    /**
     * @dev Returns the value at a given timestamp. If a checkpoint is not available at that block, the closest one
    ...
    function getAtProbablyRecentTimestamp(History storage self, uint256 timestamp) internal view returns (uint256) {
        ...
        if (len > 5) {
            ...
            if (key < _unsafeAccess(self._checkpoints, mid)._timestamp) {
                high = mid;
            } else {
                low = mid + 1;
            }
     ...
     * Returns previous value and new value.
     */
    function push(History storage self, function(uint256, uint256) view returns (uint256) op, uint256 delta) internal returns (uint256, uint256) {
    ...
    function _insert(Checkpoint[] storage self, uint32 key, uint224 value) private returns (uint224, uint224) {
        ...
        if (pos > 0) {
            ...
            require(last._timestamp <= key, "Checkpoint: decreasing keys");
     ...
     */
    function _upperBinaryLookup(Checkpoint[] storage self, uint32 key, uint256 low, uint256 high) private view returns (uint256) {
        while (low < high) {
     ...
     * WARNING: `high` should not be greater than the array's length.
     */
    function _lowerBinaryLookup(Checkpoint[] storage self, uint32 key, uint256 low, uint256 high) private view returns (uint256) {
        while (low < high) {
            uint256 mid = Math.average(low, high);
            if (_unsafeAccess(self, mid)._timestamp < key) {
                low = mid + 1;
