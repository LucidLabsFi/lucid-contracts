...
abstract contract BondBaseSDA is IBondSDA, Context, Auth {
    ...
    constructor(IBondTeller teller_, IBondAggregator aggregator_, address guardian_, Authority authority_) Auth(guardian_, authority_) {
    ...
    function _createMarket(MarketParams memory params_) internal returns (uint256) {
        {
            ...
            if (!callbackAuthorized[_msgSender()] && params_.callbackAddr != address(0)) revert Auctioneer_NotAuthorized();
        ...
        {
            // Check time bounds
            if (params_.duration < minMarketDuration || params_.depositInterval < minDepositInterval || params_.depositInterval > params_.duration)
            ...
            metadata[marketId] = BondMetadata({
                ...
                lastTuneDebt: (params_.capacityInQuote ? params_.capacity.mulDiv(scale, params_.formattedInitialPrice) : params_.capacity).mulDiv(
        ...
        uint256 _maxPayout;
        {
            uint256 capacity = params_.capacityInQuote ? params_.capacity.mulDiv(scale, params_.formattedInitialPrice) : params_.capacity;
        ...
        uint256 minDebtBuffer_ = _maxPayout.mulDiv(FEE_DECIMALS, targetDebt) > minDebtBuffer
            ...
            : minDebtBuffer;
        uint256 maxDebt = targetDebt + targetDebt.mulDiv(uint256(params_.debtBuffer > minDebtBuffer_ ? params_.debtBuffer : minDebtBuffer_), 1e5);
    ...
    function setIntervals(uint256 id_, uint32[3] calldata intervals_) external override {
        ...
        if (_msgSender() != market.owner) revert Auctioneer_OnlyMarketOwner();

        // Update intervals
        meta.tuneInterval = intervals_[0];
        meta.tuneIntervalCapacity = market.capacity.mulDiv(uint256(intervals_[0]), uint256(terms[id_].conclusion) - block.timestamp); // this will update tuneIntervalCapacity based on time remaining
        meta.tuneBelowCapacity = market.capacity > meta.tuneIntervalCapacity ? market.capacity - meta.tuneIntervalCapacity : 0;
    ...
    function pushOwnership(uint256 id_, address newOwner_) external override {
        if (_msgSender() != markets[id_].owner) revert Auctioneer_OnlyMarketOwner();
    ...
    /// @inheritdoc IBondAuctioneer
    function purchaseBond(uint256 id_, uint256 amount_, uint256 minAmountOut_) external override returns (uint256 payout) {
        if (_msgSender() != address(_teller)) revert Auctioneer_NotAuthorized();
        ...
        if (market.capacityInQuote ? amount_ > market.capacity : payout > market.capacity) revert Auctioneer_NotEnoughCapacity();
    ...
    function _decayAndGetPrice(uint256 id_, uint256 amount_, uint48 time_) internal returns (uint256 marketPrice_, uint256 payout_) {
        BondMarket memory market = markets[id_];
    ...
    function _tune(uint256 id_, uint48 time_, uint256 price_) internal {
        ...
        uint256 initialCapacity = capacity + (market.capacityInQuote ? market.purchased.mulDiv(market.scale, price_) : market.sold);
        ...
        if (
            ...
            if (newControlVariable < term.controlVariable) {
                ...
                adjustments[id_] = Adjustment(term.controlVariable - newControlVariable, time_, meta.tuneAdjustmentDelay, true);
            } else {
                // Tune up immediately
            ...
            metadata[id_].tuneBelowCapacity = market.capacity > meta.tuneIntervalCapacity ? market.capacity - meta.tuneIntervalCapacity : 0;
    ...
    function getMarketInfoForPurchase(
        ...
        external
        view
        returns (address owner, address callbackAddr, ERC20 payoutToken, ERC20 quoteToken, uint48[4] memory vestTerms, uint256 maxPayout_)
    {
        BondMarket memory market = markets[id_];
        return (
            market.owner,
    ...
    function currentDebt(uint256 id_) public view override returns (uint256) {
        ...
        if (lastDecay > currentTime) {
            ...
            return markets[id_].totalDebt.mulDiv(uint256(meta.debtDecayInterval) + secondsUntil, uint256(meta.debtDecayInterval));
        } else {
            ...
            return
                secondsSince > meta.debtDecayInterval
                    ? 0
                    : markets[id_].totalDebt.mulDiv(uint256(meta.debtDecayInterval) - secondsSince, uint256(meta.debtDecayInterval));
        }
    }
    ...
    function isLive(uint256 id_) public view override returns (bool) {
        return (markets[id_].capacity != 0 && terms[id_].conclusion > uint48(block.timestamp) && terms[id_].start <= uint48(block.timestamp));
    }
