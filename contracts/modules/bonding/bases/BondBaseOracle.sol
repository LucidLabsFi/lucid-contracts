...
///
/// @author Oighty
abstract contract BondBaseOracle is Context, OwnableInit, IBondOracle {
    /* ========== ERRORS ========== */
    ...
    /* ========== CONSTRUCTOR ========== */
    constructor(address aggregator_, address[] memory auctioneers_, address _owner) OwnableInit(_owner) {
    ...
    function registerMarket(uint256 id_, ERC20 quoteToken_, ERC20 payoutToken_) external virtual override {
        // Confirm that call is from supported auctioneer
        if (!isAuctioneer[_msgSender()]) revert BondOracle_NotAuctioneer(_msgSender());

        // Confirm that the calling auctioneer is the creator of the market ID
        if (address(aggregator.getAuctioneer(id_)) != _msgSender()) revert BondOracle_InvalidParams();

        // Confirm that the quote token : payout token pair is supported
        if (!supportedPairs[quoteToken_][payoutToken_]) revert BondOracle_PairNotSupported(quoteToken_, payoutToken_);
    ...
    function currentPrice(uint256 id_) external view virtual override returns (uint256) {
        ...
        if (address(tokens[0]) == address(0) || address(tokens[1]) == address(0)) revert BondOracle_MarketNotRegistered(id_);
    ...
    /// @inheritdoc IBondOracle
    function currentPrice(ERC20 quoteToken_, ERC20 payoutToken_) external view virtual override returns (uint256) {
        // Check that the pair is supported by the oracle
        if (address(quoteToken_) == address(0) || address(payoutToken_) == address(0) || !supportedPairs[quoteToken_][payoutToken_])
    ...
    function _currentPrice(ERC20 quoteToken_, ERC20 payoutToken_) internal view virtual returns (uint256);
    ...
    /// @inheritdoc IBondOracle
    function decimals(ERC20 quoteToken_, ERC20 payoutToken_) external view virtual override returns (uint8) {
        // Check that the pair is supported by the oracle
        if (address(quoteToken_) == address(0) || address(payoutToken_) == address(0) || !supportedPairs[quoteToken_][payoutToken_])
            revert BondOracle_PairNotSupported(quoteToken_, payoutToken_);
    ...
    function setPair(ERC20 quoteToken_, ERC20 payoutToken_, bool supported_, bytes calldata oracleData_) external onlyOwner {
        // Don't allow setting tokens to zero address
        if (address(quoteToken_) == address(0) || address(payoutToken_) == address(0)) revert BondOracle_InvalidParams();
    ...
    function _setPair(ERC20 quoteToken_, ERC20 payoutToken_, bool supported_, bytes memory oracleData_) internal virtual;
}
