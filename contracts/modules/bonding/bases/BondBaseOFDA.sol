...
///
/// @author Oighty
abstract contract BondBaseOFDA is IBondOFDA, Context, Auth {
    ...
    function _createMarket(MarketParams memory params_) internal returns (uint256) {
        ...
        if (params_.fixedDiscount >= ONE_HUNDRED_PERCENT || params_.fixedDiscount > params_.maxDiscountFromCurrent) revert Auctioneer_InvalidParams();
        ...
        uint256 capacity = params_.capacityInQuote
            ? params_.capacity.mulDiv(scale, price.mulDivUp(uint256(ONE_HUNDRED_PERCENT - params_.fixedDiscount), uint256(ONE_HUNDRED_PERCENT)))
            : params_.capacity;
    ...
    function payoutFor(uint256 amount_, uint256 id_, address referrer_) public view override returns (uint256) {
        ...
        uint256 fee = amount_.mulDiv(_teller.getFee(issuer, referrer_), ONE_HUNDRED_PERCENT);
