...
///
/// @author Oighty
abstract contract BondBaseOSDA is IBondOSDA, Context, Auth {
    ...
    function _createMarket(MarketParams memory params_) internal returns (uint256) {
        ...
        if (params_.baseDiscount >= ONE_HUNDRED_PERCENT || params_.baseDiscount > params_.maxDiscountFromCurrent) revert Auctioneer_InvalidParams();
        ...
        term.minPrice = price.mulDivUp(uint256(ONE_HUNDRED_PERCENT - params_.maxDiscountFromCurrent), uint256(ONE_HUNDRED_PERCENT));
        ...
        uint256 capacity = params_.capacityInQuote
            ? params_.capacity.mulDiv(scale, price.mulDivUp(uint256(ONE_HUNDRED_PERCENT - params_.baseDiscount), uint256(ONE_HUNDRED_PERCENT)))
            : params_.capacity;
        ...
        term.linearDuration = params_.linearDuration;
        term.cliffDuration = params_.cliffDuration;
    ...
    function _currentMarketPrice(uint256 id_) internal view returns (uint256) {
        ...
        if (expectedCapacity > market.capacity) {
            adjustment = ONE_HUNDRED_PERCENT + (term.decaySpeed * (expectedCapacity - market.capacity)) / initialCapacity;
        } else {
    ...
    function maxAmountAccepted(uint256 id_, address referrer_) external view returns (uint256) {
        ...
        uint256 estimatedFee = amountAccepted.mulDiv(_teller.getFee(market.owner, referrer_), ONE_HUNDRED_PERCENT);
