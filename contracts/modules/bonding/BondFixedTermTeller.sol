...
contract BondFixedTermTeller is BondBaseTeller, IBondFixedTermTeller, ERC1155 {
    ...
    function create(ERC20 underlying_, uint48 expiry_, uint256 amount_) external override nonReentrant returns (uint256, uint256) {
        ...
        if (protocolFee > createFeeDiscount) {
            ...
            _mintToken(_msgSender(), tokenId, amount_ - feeAmount);

            return (tokenId, amount_ - feeAmount);
        } else {
            // Mint new bond tokens
            _mintToken(_msgSender(), tokenId, amount_);
