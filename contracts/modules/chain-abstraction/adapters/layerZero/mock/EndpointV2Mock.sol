...
import {OFTMsgCodec} from "../layerZero/libs/OFTMsgCodec.sol";
import {Errors} from "@layerzerolabs/lz-evm-protocol-v2/contracts/libs/Errors.sol";
import {GUID} from "@layerzerolabs/lz-evm-protocol-v2/contracts/libs/GUID.sol";
import {ExecutorOptions} from "@layerzerolabs/lz-evm-protocol-v2/contracts/messagelib/libs/ExecutorOptions.sol";
import {PacketV1Codec} from "@layerzerolabs/lz-evm-protocol-v2/contracts/messagelib/libs/PacketV1Codec.sol";
import {WorkerOptions} from "../layerZero/libs/SendLibBase.sol";
import {IExecutorFeeLib} from "../layerZero/interfaces/IExecutorFeeLib.sol";
import {DVNOptions} from "../layerZero/libs/DVNOptions.sol";
import {UlnOptions} from "../layerZero/libs/UlnOptions.sol";
...
contract EndpointV2Mock is ILayerZeroEndpointV2, MessagingContext {
    ...
    mapping(address receiver => mapping(uint32 srcEid => mapping(bytes32 sender => uint64 nonce))) public lazyInboundNonce;
    ...
    function send(
        ...
        Origin memory origin = Origin({srcEid: packet.srcEid, sender: packet.sender.addressToBytes32(), nonce: packet.nonce});
    ...
    function receivePayload(
        ...
        if (_msgValue > 0) {
            try ILayerZeroReceiver(_receiver).lzReceive{value: _msgValue, gas: _gas}(_origin, _guid, _message, address(0), "") {} catch (
                bytes memory /*reason*/
            ) {}
        } else {
            try ILayerZeroReceiver(_receiver).lzReceive{gas: _gas}(_origin, _guid, _message, address(0), "") {} catch (bytes memory /*reason*/) {}
        }
    }
    ...
    function _quote(MessagingParams calldata _params, address /*_sender*/) internal view returns (MessagingFee memory messagingFee) {
    ...
    function _decodeExecutorOptions(bytes calldata _options) internal view returns (uint256 dstAmount, uint256 totalGas) {
    ...
    function decode(bytes calldata _options) internal pure returns (bytes memory executorOptions, bytes memory dvnOptions) {
    ...
    function _insertWorkerOptions(
        ...
        if (_workerId == ExecutorOptions.WORKER_ID) {
            _executorOptions = _executorOptions.length == 0 ? _newOptions : abi.encodePacked(_executorOptions, _newOptions);
    ...
    function decodeLegacyOptions(uint16 _optionType, bytes calldata _options) internal pure returns (bytes memory executorOptions) {
    ...
    mapping(address from => mapping(address to => mapping(bytes32 guid => mapping(uint16 index => bytes32 messageHash)))) public composeQueue;
    ...
    function getConfig(address /*_oapp*/, address /*_lib*/, uint32 /*_eid*/, uint32 /*_configType*/) external pure returns (bytes memory config) {
        return bytes("0x");
    }

    function getReceiveLibrary(address /*receiver*/, uint32 /*_eid*/) external pure returns (address lib, bool isDefault) {
    ...
    function nextGuid(address /*_sender,*/, uint32 /*_dstEid,*/, bytes32 /*_receiver*/) external pure returns (bytes32) {
        return 0;
    }
    ...
    // Helper Functions
    function executeNativeAirDropAndReturnLzGas(bytes calldata _options) public returns (uint256 totalGas, uint256 dstAmount) {
        (bytes memory executorOpts, ) = decode(_options);
        return this._executeNativeAirDropAndReturnLzGas(executorOpts);
    }

    function _executeNativeAirDropAndReturnLzGas(bytes calldata _options) public returns (uint256 totalGas, uint256 dstAmount) {
        if (_options.length == 0) {
            revert IExecutorFeeLib.Executor_NoOptions();
        }
    ...
    function _initializable(Origin calldata _origin, address _receiver, uint64 _lazyInboundNonce) internal view returns (bool) {
        return
