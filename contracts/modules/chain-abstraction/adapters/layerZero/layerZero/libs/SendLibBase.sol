...
abstract contract SendLibBase is MessageLibBase, Ownable {
    ...
    constructor(address _endpoint, uint32 _localEid, uint256 _treasuryGasLimit, uint256 _treasuryNativeFeeCap) MessageLibBase(_endpoint, _localEid) {
    ...
    function setTreasuryNativeFeeCap(uint256 _newTreasuryNativeFeeCap) external onlyOwner {
        ...
        if (_newTreasuryNativeFeeCap > treasuryNativeFeeCap) revert LZ_MessageLib_InvalidAmount(_newTreasuryNativeFeeCap, treasuryNativeFeeCap);
    ...
    function _payTreasury(
        ...
        if (treasury != address(0x0)) {
            bytes memory callData = abi.encodeCall(ILayerZeroTreasury.payFee, (_sender, _dstEid, _totalNativeFee, _payInLzToken));
    ...
    function _quoteTreasury(
        ...
        if (treasury != address(0x0)) {
            bytes memory callData = abi.encodeCall(ILayerZeroTreasury.getFee, (_sender, _dstEid, _totalNativeFee, _payInLzToken));
            (bool success, bytes memory result) = treasury.safeStaticCall(treasuryGasLimit, TREASURY_MAX_COPY, callData);
    ...
    function _quoteVerifier(address _oapp, uint32 _eid, WorkerOptions[] memory _options) internal view virtual returns (uint256 nativeFee);
