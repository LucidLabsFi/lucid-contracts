...
abstract contract Deployer is IDeployer {
     ...
     */
    // slither-disable-next-line locked-ether
    function deployAndInit(bytes memory bytecode, bytes32 salt, bytes calldata init) external payable returns (address deployedAddress_) {
        bytes32 deploySalt = keccak256(abi.encode(msg.sender, salt));
        deployedAddress_ = _deployedAddress(bytecode, deploySalt);
     ...
     */
    function deployedAddress(bytes memory bytecode, address sender, bytes32 salt) public view returns (address) {
